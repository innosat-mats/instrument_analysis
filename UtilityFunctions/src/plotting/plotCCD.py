...
def simple_plot(CCDitems, outdir, nstd=2, cmap='inferno', custom_cbar=False,
    ...
    for CCDno in range(0,8):
        ...
        for index,CCD in CCDs.iterrows():
            ...
            # orbital parameters
            satlat,satlon,satLT,nadir_sza,nadir_mza,TPlat,TPlon,TPLT,TPsza,TPssa = satellite.get_position(CCD['EXP Date'])

            # plot CCD image
            if custom_cbar:
                plt.pcolormesh(image,cmap=cmap, vmax=ranges[1], vmin=ranges[0])
            else:
                plt.pcolormesh(image,cmap='magma', vmax=mean+nstd*std, vmin=mean-nstd*std)
            plt.colorbar(label='counts')


            # print out additional information
            plt.figtext(0.1, 0.8, f'tpSZA: {TPsza:.6}', fontsize=10, color='white')
            plt.figtext(0.5, 0.8, f'satlat, satlon: ({satlat:.6}, {satlon:.6})', fontsize=10,color='white')
            plt.figtext(0.25, 0.8, f'TPlat, TPlon: ({TPlat:.6}, {TPlon:.6})', fontsize=10,color='white')
    ...
    for CCDno in range(0,8):
        ...
        if len(CCDs) > 0:
            ...
            for index,CCD in CCDs.iterrows():
                ...
                ax = plt.subplot2grid((2, 2), (1, 0), colspan=1, rowspan=1, projection=ccrs.PlateCarree(),fig=fig)
                ax1 = plt.subplot2grid((2, 2), (0, 0),rowspan=1, colspan=2,fig=fig)
                ax2 = plt.subplot2grid((2, 2), (1,1),rowspan=1, colspan=1,fig=fig)

                # map settings
                gl = ax.gridlines(crs=ccrs.PlateCarree(), draw_labels=True,
                                linewidth=0.9, color='black', alpha=0.5, linestyle='-')
                gl.xlabels_top = True
                ...
                # plot CCD image
                if custom_cbar:
                    img=ax1.pcolormesh(image,cmap=cmap, vmax=ranges[1], vmin=ranges[0])
                else:
                    img=ax1.pcolormesh(image,cmap='magma', vmax=mean+nstd*std, vmin=mean-nstd*std)
                ax1.set_title(f'ch: {channel}; time: {exp_date}; TEXPMS: {texpms}')
                fig.colorbar(img,ax=ax1)

                # plot sat position and tangent point
                ax.scatter(satlon,satlat,s=10, color='red', label='satellite pos.')
                ax.scatter(TPlon,TPlat,s=10, color='green', label='TP pos.')
                ax.legend(ncol=2, fontsize=7, loc='lower right')

                # print out additional information
                plt.figtext(0.15, 0.03, f'nadirSZA: {nadir_sza:.6}', fontsize=10)
                plt.figtext(0.15, 0.06, f'nadirMZA: {nadir_mza:.6}', fontsize=10)
                ...
                ax2.hist(image.flatten(), bins=nbins, alpha=0.6, density=True,range=[mean-nstd*std, mean+nstd*std])
                ax2.set_xlabel('counts')
                ax2.axvline(x=mean,label='mean',linestyle='--', linewidth=1.5)
