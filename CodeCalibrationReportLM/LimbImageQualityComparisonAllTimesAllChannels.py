...
for ichannel, channel in enumerate(channellist):
    ...
    cooltest=True
    if cooltest==True: # For Cooltest 
        picdictlistselect=[picdictlist[i] for i in [4,5,7]]    
    else:    #For changes in time due to vibrations 
    ...
    for index, picdict in enumerate(picdictlistselect):
        ...
        if min((res_h-dark)/(bright-dark))>0: 
...
if cooltest==True: # For Cooltest 
    fig3.savefig(output_imagedir+'image_resolution_cooltest_all_channels.jpg')
else:       
    fig3.savefig(output_imagedir+'image_resolution_alltime_all_channels.jpg')
