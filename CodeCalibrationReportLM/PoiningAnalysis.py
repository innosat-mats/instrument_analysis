...
def findpattern(lena,template):
    ...
    return x , y, bestcorrpic, res_h, res_v, bright, dark, fig
...
AITpic=2**4*np.int64(Image.open(imagedir + 'output'+str(pics[2])+'.pnm'))
AITpic_after_glue=2**4*np.int64(Image.open(afterglue + 'output'+str(gpics[0])+'.pnm'))
...
templateorg=np.fliplr(np.copy(templatepic[170:244,820:940]))



tempfig, tempaxes = plt.subplots(1)
sp1=plot_CCDimage(templateorg,tempfig, tempaxes,title='template Woojin IR2')  
...
namelist=['March 2018','April 2018','May 2019 ', 'Sept 2019', 'Jan 2020 22 C','Jan 2020 14C','Jan 2020 11C','Jan 2020 5 C', 'Feb 2021' ]
leftrotated=[False, True, False, False, True, True, True, True, False]
rightrotated=[False,False, True, True, False, False, False, False, False]
picdictlist=[]
for index, pic in  enumerate(piclist):
    picdict= {
        "pic": pic,
        "name": namelist[index],
        "leftrotated": leftrotated[index],
        "rightrotated": rightrotated[index],
        }
    picdictlist.append(picdict)    
picdictlistselect=[picdictlist[i] for i in [0,1,2,3,4,8]]
...
for index, picdict in enumerate(picdictlistselect):
    ...
    #mean=pic1[:,100:2000].mean() 
    #std=pic1[:,100:2000].std() 

    #clim=[mean-3*std, mean+3*std]    
    plot_CCDimage(pic1,fig, axes[index],title=picdict['name']+' '+channel)
...
for index, picdict in enumerate(picdictlistselect):
    ...
    if picdict['rightrotated']:    
        template=np.rot90(templateorg,3)

    elif picdict['leftrotated']:    
        template=np.rot90(templateorg,1)        
    else:
        template=templateorg

    x, y, bestcorrpic, res_h, res_v, bright, dark, figure= findpattern(pic_input,template)
    figure.suptitle(picdict['name'])
    
    print(picdict['name'],' ',channel,' x: ',x,'y: ',y )#, ' xdiff:', x-, ' ydif' )
    pointdict= {
        "channel": channel,
        "name": namelist[index],
        "x": x,
    ...
    pointdictlist.append(pointdict)  
    
    if picdict['rightrotated']  or picdict['leftrotated']: #swich horizontla and vertical resolution
        res_v_temp=res_v
        res_v=res_h
        res_h=res_v_temp
        
 
    sp=axes3[0].plot((res_h-dark)/(bright-dark),label=picdict['name'])
    axes3[0].set_title('Horizonal resolution '+channel)
    axes3[0].legend()
    sp=axes3[1].plot((res_v-dark)/(bright-dark),label=picdict['name'])   
    axes3[1].set_title('Vertical resolution '+channel)
    axes3[1].legend()
    if leftrotated[index] or rightrotated[index]:
