...
def makeplot(number, starttime):
    ...
    tle = ['1 26702U 01007A   23037.91034836  .00007274  00000+0  42696-3 0  9993',
           '2 26702  97.5120  53.5306 0009881 191.0534 169.0481 15.11989832201676']
    ...
    ascnod /= norm(ascnod)
    arg = np.array([np.rad2deg(np.arccos(np.dot(-mrunit[:, i], ascnod.T)))
                   * np.sign(-mrunit[2, i]) for i in range(vunit.shape[1])]).T
    yaw = 0
    yawoffset = 0
    pitch = -20.86  # deg
    print(pitch)
    ...
    for i in range(vunit.shape[1]):
        FOVi = R.from_euler('XYZ', [0, pitch, yaw[i]],
    ...
    TPssa = (np.rad2deg(np.arccos(
        np.dot(FOV.T, (sundir.position.m/norm(sundir.position.m, axis=0))))))[:, 0]
    ...
    plt.xlabel('Solar zenith angle')
    plt.title(
        'Limb Tangent point SZA/SSA  DOY {}'.format(times[0].timetuple().tm_yday))
    plt.legend(['SZA', 'SSA'])
    clinecolour = plt.gca().get_lines()[0].get_color()
