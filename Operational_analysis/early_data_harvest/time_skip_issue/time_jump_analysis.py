...
#         df_ccd.at[j,"EXP Nanoseconds"] = df_ccd.at[j,"EXP Nanoseconds"]-timediff[j]*1e9
#         print(df_ccd.at[j,"EXP Nanoseconds"])
...
# plt.show()
# timediff_2 = df_ccd["EXP Nanoseconds"].diff()*1e-9-TEXPIMS_CCD
# plt.plot(timediff)
# plt.plot(timediff_2,'.')
# plt.ylabel('Difference in time between images [s]')
# plt.xlabel('Image number')
# plt.legend(['Before correction','After correction'])
# plt.title('Timestamp stability CCD ' + str(CCD) + ' pass: ' + str(orbit))
# plt.show()

# print(np.max(np.abs(timediff_2))<1e-3)
# print(np.max(np.abs(timediff))<1e-3)
# # %%



def test_timecorrection():
    ...
    df["TMHeaderTime"] = pd.to_datetime(df["TMHeaderTime"])
    TEXPIMS = [10.7,10.7,10.7,10.7,10.7,10.7,4]
    tolerance = 0.5 #tolerance of 0.5s
    
    for CCD in [1,2,3,4,5,6,7]:
        df_ccd = df[df["CCDSEL"] == CCD]
        df_ccd = df_ccd.reset_index()
        correct_time(df_ccd, TEXPIMS[CCD-1],0.5)
        timediff = df_ccd["EXP Nanoseconds"].diff()*1e-9-TEXPIMS[CCD-1]
        
        #Check that no timediff is large than tolerance
        assert (np.max(np.abs(timediff))<tolerance)



def correct_time(df, expected_diff, tolerance):

    #Calculate the difference between expected time 
    #difference and true time difference.
    timediff = df["EXP Nanoseconds"].diff()*1e-9-expected_diff
    timeshift_index = timediff[np.abs(timediff)>tolerance].index
    
    #Find longest time where the timing is stable
    long_stab_time = np.diff(timeshift_index).argmax()

    #Correct backwards from longest time
    for i in range(0,long_stab_time):
        index_of_shift_end = timeshift_index[long_stab_time-i]
        index_of_shift_start = timeshift_index[long_stab_time-i-1]
        for j in range(index_of_shift_start,index_of_shift_end):
            df.at[j,"EXP Nanoseconds"] = df.at[j,"EXP Nanoseconds"]-timediff[j]*1e9

    #Correct forward from longest time
    for i in range(long_stab_time+1,len(timeshift_index)-1):
        index_of_shift_end = timeshift_index[i+1]
        index_of_shift_start = timeshift_index[i]
        for j in range(index_of_shift_start,index_of_shift_end):
